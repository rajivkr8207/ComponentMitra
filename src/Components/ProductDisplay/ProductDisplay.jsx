...
const ProductDisplay = (props) => {
    const [currentImage, setCurrentImage] = useState('http://localhost:5173/src/Components/Assets/product_14.png');
    const {product} = props
        ...
        <div className="flex flex-col md:flex-row p-4">
            ...
            <div className="w-full md:w-1/2 my-3">
                <div className="slider-container">
                    <Slider {...settings}>
                        <div>
                            <img src={product.image} alt="Product" className="lg:w-[80%] md:w-[80%] w-[100%] mx-auto h-[25rem]" />
                        </div>
                        <div>
                            <img src={product.image} alt="Product" className="lg:w-[80%] md:w-[80%] w-[100%] mx-auto h-[25rem]" />
                        </div>
                        <div>
                            <img src={product.image} alt="Pr oduct" className="lg:w-[80%] md:w-[80%] w-[100%] mx-auto h-[25rem]" />
                        </div>
                        <div>
                            <img src={product.image} alt="Product" className="lg:w-[80%] md:w-[80%] w-[100%] mx-auto h-[25rem]" />
                        </div>
            ...
            <div className="w-full md:w-1/2 mt-4 md:mt-0 md:ml-8">
                ...
                <div className="flex items-center space-x-4 mb-2">
                    <span className="text-xl font-semibold text-[--secondary-color]">₹{product.new_price}</span>
                    <span className="text-lg text-gray-500 line-through">₹{product.old_price}</span>
                </div>

                {/* Product Short Description */}
                <div className='text-base font-normal my-3'>
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Earum omnis iusto quibusdam quas, animi rem?
                </div>
               ...
               <div className='flex justify-start items-center gap-5 mb-4'>
                    {isInCart ? (
                        <Link to="/cart" className="bg-[--primary-color] text-white py-3 px-6 rounded-md hover:bg-[--secondary-color]">
                            Go to Cart
                        </Link>
                    ) : (
                        <button onClick={() => { addToCart(product.id) }} className="bg-[--primary-color] text-white py-3 px-6 rounded-md hover:bg-[--secondary-color]">
                            Add to Cart
                        </button>
                    )}
                    <button className="border border-gray-300 py-3 px-4 group rounded-md hover:bg-gray-100">
                        <FaHeart size={20} className='text-gray-400 group-hover:text-red-500' />
                    </button>
                </div>
                ...
                <div className="border border-gray-200 rounded-md mb-4">
                    ...
                    <div className="flex px-3 py-4 items-center gap-3 border-t border-gray-200">
                        ...
                        <div>
                            ...
                            <p className='text-sm'>7 Day Return Policy <span className='cursor-pointer text-blue-600 underline'>Details</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p className="text-gray-700">This is a detailed description of the product. It explains the features, benefits, and other important information about the product.</p>
                </div>

                {/* Customer Reviews */}
                <div className="mt-6">
                    <h2 className="text-lg font-semibold mb-2">Customer Reviews</h2>
                    <p className="text-sm text-gray-700">John Doe: "Great product! Highly recommend it."</p>
                    <p className="text-sm text-gray-700">Jane Smith: "Good quality for the price."</p>
                    <p className="text-sm text-blue-600 cursor-pointer">Read more reviews</p>
                </div>
            </div>
        </div>
