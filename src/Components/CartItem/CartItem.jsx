...
const CartItem = () => {
    const { getTotalCartAmount, allProduct, cartitems, removeFromCart, addToCart ,removeAllFromCart} = useContext(ShopContext);
                        ...
                        {isCartEmpty ? (
                            <div className="flex items-center justify-center">
                                <div className="bg-white p-8 rounded-lg shadow-md text-center">
                                    ...
                                    <Link to='/' className="px-6 py-2 bg-[--primary-color] text-white rounded-md hover:bg-[--secondary-color]">
                                        Start Shopping
                                    </Link>
                                </div>
                            </div>
                            ...
                            allProduct.map((item) => {
                                if (cartitems[item.id]) {
                                    ...
                                    const itemTotalPrice = item.new_price * itemQuantity;
                                    return (

                                        <div key={item.id} className="flex  flex-row max-w-3xl mx-auto pr-3  rounded-lg mb-4 bg-[#ffffff] shadow-md hover:shadow-xl transition duration-300 transform hover:scale-105">
                                        ...
                                        <Link to={`/product/${item.id}`}>
                                            ...
                                            <div className="w-full lg:w-1/2 py-2 flex justify-between">
                                                ...
                                                <Link to={`/product/${item.id}`}>
                                                    <h2 className="lg:text-xl text-md font-bold">{item.name}</h2>
                                                    <div className="flex items-center mt-2">
                                                        <span className="text-lg font-bold text-[--primary-color]">₹{item.new_price}</span>
                                                        <span className="ml-2 text-sm text-gray-500 line-through">₹{item.old_price}</span>
                                                    </div>
                                                    </Link>
                                                    <div className="flex items-center mt-4">
                                                        ...
                                                        <span className="text-lg font-bold mx-2">{itemQuantity}</span>
                                                        <button
                                                ...
                                                <div>
                                                    <div className="flex flex-col items-center">
                                                        <h3 className="lg:text-lg text-base font-bold">Total:</h3>
                                                        <span className="lg:text-4xl text-3xl text-center font-bold text-[--secondary-color]">₹{itemTotalPrice}</span>
                                                    </div>
